...
public class MainActivity extends AppCompatActivity {
    ...
    private void loadProfile(Intent data) {

        int left = data.getIntExtra("offsetX", 0);
        int top = data.getIntExtra("offsetY", 0);
        int width = data.getIntExtra("w", 0);
        int height = data.getIntExtra("h", 0);
        Uri uri = data.getParcelableExtra("path");
        Uri original = Uri.parse(data.getStringExtra("original"));

        Picasso.get().load(original)
                .transform(
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        if (requestCode == REQUEST_IMAGE) {
            if (resultCode == Activity.RESULT_OK) {
                Uri uri = data.getParcelableExtra("path");
                Uri orginal = Uri.parse(data.getStringExtra("original"));
                Log.i("SARA", "" + data.getIntExtra("offsetX", 0));
